...
function getAvailablePort(startPort = 3123) {
...
if (PROXY_MODE && IS_DEVELOPMENT) {
  // En modo proxy, redirigir a Vite dev server para rutas no-API
...
// Iniciar servidor
getAvailablePort(process.env.PORT || 3123).then(PORT => {
  app.listen(PORT, () => {
    console.log(`Server running on http://localhost:${PORT}`);
    console.log(`Environment: ${IS_DEVELOPMENT ? 'development' : 'production'}`);
    console.log(`Mode: ${PROXY_MODE ? 'proxy' : 'static'}`);
  });
}).catch(err => {
